...
vector_rag.py
─────────────
Phase 3: Vector RAG for Sopra HR ticket resolution.
Vector store: FAISS (local, no server, Python 3.14 compatible)
Embeddings:   sentence-transformers (multilingual, local, free)
Generator:    Amazon Bedrock (Claude Haiku)


Usage:
    # Step 1: Index your tickets (run once)
    python vector_rag.py index --tickets ./data/cleaned --db ./faiss_db

    # Step 2: Test a single question
    ...
    # Step 4: Evaluate against your eval set
...
def ticket_to_document(ticket: dict) -> dict:
    parts = []
    if ticket.get("title"):       parts.append(f"Titre: {ticket['title']}")
    if ticket.get("version"):     parts.append(f"Version: {ticket['version']}")
    if ticket.get("system"):      parts.append(f"Système: {ticket['system']}")
    if ticket.get("description"): parts.append(f"Problème: {ticket['description'][:600]}")
    if ticket.get("resolution"):  parts.append(f"Résolution: {ticket['resolution'][:500]}")
    if ticket.get("patches"):     parts.append(f"Patches: {', '.join(ticket['patches'])}")

    return {
        "text":        "\n".join(parts),
        "reference":   ticket.get("reference", ""),
        "title":       ticket.get("title", "")[:200],
...
def index_tickets(tickets_dir: str, db_path: str, batch_size: int = 256):
    ...
    for i in range(0, len(docs), batch_size):
        ...
        print(f"  Batch {batch_num:3d}/{total_batches} — {min(i+batch_size, len(docs))}/{len(docs)} done")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Vector RAG — Sopra HR (FAISS)")
